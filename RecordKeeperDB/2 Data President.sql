...
union select 45, 'Donald', 'Trump', (select z.PartyId from Party z where z.PartyName =  'Republican'), '1946-01-01', null, 2016, 2020
union select 46, 'Joe', 'Biden', (select z.PartyId from Party z where z.PartyName =  'Democrat'), '1942-01-01', null, 2020, null
...
union select (select p.PresidentId from President p where p.Num = 33), 10340, 3139, 1952, 'Directing the Secretary of Commerce To Take Possession of and Operate the Plants and Facilities of Certain Steel Companies.', 0
union select (select p.PresidentId from President p where p.Num = 44), 13759, 5323, 2017, 'Designating the World Organisation for Animal Health as a Public International Organization Entitled to Enjoy Certain Privileges, Exemptions, and Immunities.', 1
union select (select p.PresidentId from President p where p.Num = 44), 13762, 7619, 2017, 'Providing an Order of Succession Within the Department of Justice.', 1
union select (select p.PresidentId from President p where p.Num = 40), 12288, 10135, 1981, 'Termination of the Wage and Price Regulatory Program.', 1
