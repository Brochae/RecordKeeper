...
go
		...
		TermEnd int,
        FullColorCode varchar (100) not null,
...
delete president
insert president(Num, FirstName, LastName, Party, dateBorn, DateDied, TermStart, TermEnd, FullColorCode, ColorCode, ColorCode2)
...
union select 45, 'Donald', 'Trump', 'Republican', '1946-01-01', null, 2016, 2020,'','',''
union select 46, 'Joe', 'Biden', 'Democrat', '1942-01-01', null, 2020, null,'','',''

update p
set ColorCode = 'Red - '
from president p 
where p.Num between 1 and 7

update p
set ColorCode = 'Orange - '
from president p 
where p.Num between 8 and 14

update p
set ColorCode = 'Yellow - '
from president p 
where p.Num between 15 and 21

update p
set ColorCode = 'Green - '
from president p 
where p.Num between 22 and 28

update p
set ColorCode = 'Blue - '
from president p 
where p.Num between 29 and 35

update p
set ColorCode = 'Indigo - '
from president p 
where p.Num between 36 and 42

update p
set ColorCode = 'Violet - '
from president p 
where p.Num between 43 and 49

update p
set ColorCode2 = 'Red' from president p where p.Num in (1, 8, 15, 22, 29, 36, 43)

update p
set ColorCode2 = 'Orange' from president p where p.Num in (2, 9, 16, 23, 30, 37, 44)

update p
set ColorCode2 = 'Yellow' from president p where p.Num in (3, 10, 17, 24, 31, 38, 45)

update p
set ColorCode2 = 'Green' from president p where p.Num in (4, 11, 18, 25, 32, 39, 46)

update p
set ColorCode2 = 'Blue' from president p where p.Num in (5, 12, 19, 26, 33, 40, 47)

update p
set ColorCode2 = 'Indigo' from president p where p.Num in (6, 13, 20, 27, 34, 41, 48)

update p
set ColorCode2 = 'Violet' from president p where p.Num in (7, 14, 21, 28, 35, 42, 49)

update p
set FullColorCode = CONCAT(p.ColorCode, p.ColorCode2)
from president p
